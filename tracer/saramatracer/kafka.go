...
//   - **SkyWalking sw8** (go2sky): sw8 header（向后兼容）
//
// **提取优先级**：
//   1. 优先提取 traceparent（W3C 标准）
//   2. 如果没有 traceparent，尝试提取 sw8（SkyWalking）
//   3. 都没有则创建新的 trace_id
//
// 参数：
//   - message: Kafka 消费者消息
...
func CreateKafkaConsumerContext(message *sarama.ConsumerMessage) context.Context {
	...
	// 这样可以同时支持 SkyWalking (sw8) 和 Mock (trace-id) 协议
...
// stringCarrier 实现 Carrier 接口，用于字符串传递
type stringCarrier struct {
	value string
}

func (c *stringCarrier) Set(key, value string) {
	if key == "trace-id" {
		c.value = value
	}
}

func (c *stringCarrier) Get(key string) (string, bool) {
	if key == "trace-id" {
		return c.value, true
	}
	return "", false
...
func InjectKafkaProducerHeaders(ctx context.Context, msg *sarama.ProducerMessage) context.Context {
	...
	if msg.Key != nil {
		// Key 是 Encoder 接口，转换为字节后转为字符串用于日志
		if encoder, ok := msg.Key.(sarama.Encoder); ok {
			if keyBytes, err := encoder.Encode(); err == nil {
				span.SetTag("kafka.key", string(keyBytes))
			}
		}
	}
